...
if (!Array.isArray(trails) || trails.length !== 61) throw new Error(`Expected 61 statewide trails for detail-page generation, found ${trails.length}`);
...
function sourcePanel(source, official) {
  ...
  const action = sourceUrl ? `<a class="trail-verify" href="${esc(sourceUrl)}" target="_blank" rel="noopener">${live ? `Open ${esc(source.provider)}` : 'Check status source'}</a>` : '';
  const note = live
  ...
  return `<section class="trail-panel" style="margin-top:18px"><h2>Grooming &amp; status sources</h2><p><strong>${esc(source.label || 'Status source')}</strong><br>${esc(source.provider || 'Operator / land manager')}</p>${action}<p style="margin-top:14px;font-size:.82rem;color:var(--muted)">${esc(note)}</p>${official && sourceUrl !== official ? `<p style="margin-top:10px"><a href="${esc(official)}" target="_blank" rel="noopener">Operator / land-manager page →</a></p>` : ''}</section>`;
}
...
function pageFor(trail, details, nearby, source) {
  ...
  const schema = {
    ...
    '@graph': [
      {'@type':'SportsActivityLocation','@id':`${url}#trail`,name:trail.name,url,description:details.desc,address:{'@type':'PostalAddress',addressLocality:trail.town,addressRegion:'MI',addressCountry:'US'},geo:{'@type':'GeoCoordinates',latitude:trail.lat,longitude:trail.lon},sameAs:sameAs.length?sameAs:undefined},
      {'@type':'WebPage','@id':`${url}#webpage`,url,name:title,description,dateModified:'2026-09-13',about:{'@id':`${url}#trail`},isPartOf:{'@id':'https://xcski.chrisizworski.com/#website'},author:{'@id':'https://chrisizworski.com/#person'}}
    ]
  };

  return `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${esc(title)}</title><meta name="description" content="${esc(description)}"><link rel="canonical" href="${url}"><link rel="author" href="https://chrisizworski.com/chris-izworski/"><meta property="og:type" content="website"><meta property="og:title" content="${esc(title)}"><meta property="og:description" content="${esc(description)}"><meta property="og:url" content="${url}"><meta property="og:image" content="https://xcski.chrisizworski.com/og.png"><meta name="twitter:card" content="summary_large_image"><link rel="icon" href="/favicon.svg" type="image/svg+xml"><link rel="apple-touch-icon" href="/apple-touch-icon.png"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Bricolage+Grotesque:opsz,wght@12..96,400;12..96,600;12..96,800&family=Public+Sans:wght@400;500;600;700&display=swap" rel="stylesheet"><link rel="stylesheet" href="/trail-page.css"><script type="application/ld+json">${js(schema)}</script></head><body>
<header class="trail-top"><div class="trail-wrap"><a href="/">← Michigan XC Ski Trails &amp; Nordic Board</a></div></header><main class="trail-wrap"><section class="trail-hero"><p class="trail-kicker">Michigan cross-country ski trail</p><h1>${esc(trail.name)}</h1><p class="trail-town">${esc(trail.town)}, Michigan</p><div class="trail-chips">${chips.map(chip => `<span class="trail-chip">${esc(chip)}</span>`).join('')}</div></section><div class="trail-layout"><div><section class="trail-panel"><h2>Live XC intelligence</h2><div id="trail-live" data-lat="${trail.lat}" data-lon="${trail.lon}" data-cat="${trail.cat}"><div class="trail-live-state"><span class="trail-score-label">Loading</span><strong>Checking this trailhead…</strong><p>Snow score, fresh snow, rain, freeze/thaw, surface state, and best time-of-day window.</p></div></div></section><section class="trail-panel" style="margin-top:18px"><h2>About ${esc(trail.name)}</h2><p>${esc(details.desc)}</p>${details.official ? `<a class="trail-verify" href="${esc(details.official)}" target="_blank" rel="noopener">Verify official trail status</a>` : ''}<p style="margin-top:14px;font-size:.82rem;color:var(--muted)">Modeled weather and surface timing are screening signals. Grooming, opening, access, snowmaking, and actual trail surface must be verified with the operator or land manager.</p></section></div><aside><section class="trail-panel"><h2>Trail profile</h2><p><strong>System:</strong> ${esc(category)}</p><p><strong>Skate:</strong> ${trail.skate ? 'Yes / supported by the system profile' : 'Not listed as a skate system'}</p><p><strong>Rentals:</strong> ${trail.rentals ? 'Available / listed' : 'Not listed'}</p><p><strong>Lighted:</strong> ${trail.lit ? 'Yes' : 'No / not listed'}</p><p><strong>Status source:</strong> ${esc(source?.provider || 'No dedicated source registered')}</p></section>${sourcePanel(source, details.official)}<section class="trail-panel" style="margin-top:18px"><h2>Nearby XC options</h2><div class="trail-related">${nearby.map(n => `<a href="/trails/${n.id}/">${esc(n.name)} <small>· ${n.distance.toFixed(0)} mi</small></a>`).join('\n')}</div></section></aside></div></main><footer class="trail-footer"><div class="trail-wrap">Part of the <a href="/">Michigan XC Ski Trails</a> field tool by <a href="https://chrisizworski.com/chris-izworski/">Chris Izworski</a>. Live weather from Open-Meteo. Status-source provenance stays separate from the snow model.</div></footer><script src="/xc-intelligence.js" defer></script><script src="/trail-page.js" defer></script></body></html>`;
}
...
const sitemapUrls = [`<url><loc>https://xcski.chrisizworski.com/</loc><lastmod>2026-09-13</lastmod><changefreq>daily</changefreq><priority>1.0</priority></url>`,...trails.map(t => `<url><loc>https://xcski.chrisizworski.com/trails/${t.id}/</loc><lastmod>2026-09-13</lastmod><changefreq>daily</changefreq><priority>0.8</priority></url>`)];
await writeFile(path.join(out, 'sitemap.xml'), `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${sitemapUrls.join('\n')}\n</urlset>\n`);
console.log(`Generated ${trails.length} statewide indexable XC trail pages with shared surface/timing intelligence and audited source provenance.`);
