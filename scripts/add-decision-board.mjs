...
if (html.includes(heroPhrase)) {
  html = html.replace(
    heroPhrase,
    'plus a live statewide Michigan Nordic Board that compares snow, surface timing, weekend outlook, storm windows, and source confidence before you verify the operator or groomer report.'
  );
  ...
  <div class="ski-board">
    <div class="ski-board-head">
      <div>
        ...
        <p class="ski-board-intro">A statewide decision layer across all 61 trailheads. Snow score, modeled surface state, best time-of-day window, weekend outlook, storm-window potential, and source confidence stay separate so you can see what the engine knows and what still needs operator verification.</p>
      </div>
    ...
    <p class="ski-board-freshness" id="ski-board-freshness">Loading current weather, surface timing, weekend outlook, storm windows, and source confidence…</p>
    <div id="ski-board-outlook" class="ski-board-outlook" aria-live="polite"></div>
    <div class="ski-board-list" id="ski-board-list" aria-live="polite">
      <div class="ski-preseason"><strong>Building the board…</strong><p>Checking modeled base, fresh snow, rain, freeze/thaw, hourly temperature, wind, weekend signals, and potential storm windows.</p></div>
    </div>
...
<p><strong>The Michigan Nordic Board uses independent layers.</strong> Snow score compares modeled base and recent snow. Surface intelligence looks at rain, overnight freeze, daytime thaw, hourly temperature and wind. Best-time modeling compares morning, midday and afternoon. Weekend intelligence compares Saturday and Sunday with the same engine. Storm-window intelligence looks for meaningful forecast snow followed by cold/dry hours before thaw or rain. Source confidence reports whether the trail has a live grooming platform, a direct operator page, a land-manager reference, or no current machine-readable feed. None of those layers silently substitutes for another.</p>`;
...
console.log('Added statewide Michigan Nordic Board with reusable snow, surface, timing, weekend, storm-window, and source-confidence engines.');
