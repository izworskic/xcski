...
const description = 'Compare Michigan, Wisconsin and Minnesota cross-country ski conditions across 172 Nordic systems, including modeled snow, surface timing, day-over-day change, fresh snow and official status provenance.';
const schema = {
...
<section class="midwest-panel"><div class="midwest-board-head"><div><h2 id="midwest-board-label">Best modeled signals across the Midwest</h2><p id="midwest-freshness">Loading weather, change and provider layers…</p></div><span id="midwest-status" class="midwest-status">Loading…</span></div><div class="midwest-tabs"><button type="button" data-midwest-mode="best" aria-pressed="true">Best today</button><button type="button" data-midwest-mode="improvers" aria-pressed="false">Biggest improvers</button><button type="button" data-midwest-mode="fresh" aria-pressed="false">Fresh snow</button><button type="button" data-midwest-mode="lighted" aria-pressed="false">Lighted</button><button type="button" data-midwest-mode="skate" aria-pressed="false">Skate</button></div><div id="midwest-board"><div class="midwest-empty"><strong>Building the Midwest board…</strong><p>Checking all ${totalTrails} systems in state-local time.</p></div></div></section>
<section class="midwest-panel"><h2>How this comparison works</h2><div class="midwest-method"><div><strong>Same engine</strong><span>Michigan, Wisconsin and Minnesota use the same snow, freeze/thaw, surface and best-window intelligence so the cross-state comparison is internally consistent.</span></div><div><strong>Same local hour yesterday</strong><span>Change is reconstructed from Open-Meteo's prior-day hourly model data. No cookies, browser storage or fabricated snapshots are used.</span></div><div><strong>Official status stays separate</strong><span>Fresh authorized provider data can confirm open/closed status and raise confidence. It never rewrites the modeled snow score.</span></div></div></section>
<section class="midwest-panel"><h2>Go deeper by state</h2><div class="midwest-state-summary">${stateLinks}</div></section>
</main><footer class="midwest-footer"><div class="midwest-wrap">Midwest XC conditions by Chris Izworski. Modeled weather is a screening signal; verify grooming, opening and access with the linked operator or land manager before traveling.</div></footer>
...
console.log(`Generated Midwest XC Today across ${totalTrails} systems: Michigan ${states[0].trails.length}, Wisconsin ${states[1].trails.length}, Minnesota ${states[2].trails.length}.`);