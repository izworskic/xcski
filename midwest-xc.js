(() => {
  ...
  if (!ctx || !Array.isArray(ctx.states) || !window.XC_INTEL) return;
  ...
  async function fetchWeatherChunk(chunk, timezone) {
    ...
    const response = await fetch(XC_INTEL.forecastQuery(lats, lons, true, timezone));
  ...
  function renderBoard(rows, mode='best') {
    ...
    board.innerHTML = selected.length
      ? selected.map((row,i) => rowCard(row,i+1)).join('')
      : '<div class="midwest-empty"><strong>No meaningful matches in this view.</strong><p>The engine does not manufacture movement or snow when the model does not show it.</p></div>';
  }
  ...
  async function load() {
    try {
      ...
      const snowWatch = rows.filter(r => r.snowTomorrow >= 0.5 || r.snow24 >= 0.5);
      ...
      if (freshness) freshness.textContent = `Open-Meteo weather · same-local-hour change since yesterday · ${liveCount} fresh official provider update${liveCount===1?'':'s'} · ${pending} provider permission${pending===1?'':'s'} pending.`;

      if (offseason && !snowWatch.length) {
        if (status) status.textContent = 'Preseason mode';
        board.innerHTML = '<div class="midwest-empty"><strong>Winter rankings are paused.</strong><p>The Midwest engine is active across all 172 systems. Rankings and day-over-day movers return with meaningful winter snow signals.</p></div>';
        return;
      }

      if (status) status.textContent = 'Live Midwest board';
      renderBoard(rows,'best');
